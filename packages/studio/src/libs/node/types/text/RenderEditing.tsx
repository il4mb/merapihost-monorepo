...
import { useNodeDescendantsRef, useVisibleNodeChildren } from "@/hooks";
...
export default function RenderEditing({ root }: RenderEditingProps) {
    ...
    const nodeChildren = useVisibleNodeChildren(root);
    const descendantsRef = useNodeDescendantsRef(root);
    const caret = useCaret(root, descendantsRef);
    ...
    useWireEffect("renderCaret", () => caret.render(), [caret.render]);
    useWireEffect("selection", () => caret.selectionRef.current, [caret.selectionRef]);

    const toggleFormat = useCallback(
        (e: KeyboardEvent, command: "bold" | "italic" | "underline") => {
            e.preventDefault();
    ...
    const handleDelete = useCallback(
        (direction: "backspace" | "delete"): { anchor: number; focus: number } => {
            ...
            if (selection.anchor !== selection.focus) {
                ...
                if (firstIdx !== -1) {
                    ...
                    if (firstIdx === lastIdx) {
                        ...
                        const u = new NodeModel(first.node);
                        u.content = merged;
                        ...
                        for (let i = firstIdx + 1; i <= lastIdx; i++) {
                            newContents.delete(offsets[i].node.id);
                        }
                    }
                }
                newPos = start;
        ...
        (text: string, at: number): number => {
            ...
            if (!targetNode) {
                if (textNodes.length > 0) {
                    ...
                    u.content = (last.content || "") + text.replace(/\s+/g, "\u200B");
    ...
    const handleBlocking = useCallback(
        (direction: "left" | "right" | "all") => {
            ...
            const { anchor, focus } = caret.selectionRef.current;

            if (direction === "all") {
                caret.selectionRef.current = { anchor: 0, focus: totalLength };
            ...
            caret.selectionRef.current = { anchor, focus: newFocus };
            requestAnimationFrame(() => caret.render());
        },
        [caret],
    );
    ...
    const handleInput = useCallback(
        (e: KeyboardEvent) => {
            ...
            if (e.key === "ArrowLeft" || e.key === "ArrowRight") {
                ...
                const newPos = Math.max(0, Math.min(totalLength, caret.selectionRef.current.focus + delta));
                caret.selectionRef.current = { anchor: newPos, focus: newPos };
                requestAnimationFrame(() => caret.render());
                return;
            }
            ...
            if (e.key === "Backspace") {
                ...
                caret.selectionRef.current = sel;
                requestAnimationFrame(() => caret.render());
                return;
            }
            if (e.key === "Delete") {
